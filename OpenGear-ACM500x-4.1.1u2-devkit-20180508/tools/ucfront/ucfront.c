...
static void process_args(int argc, char **argv)
{
	...
	for (i=1; i<argc; i++) {
		...
		if (getenv("OPENSSL_NEEDS_DL") && strncmp(argv[i], "-lcrypto", 5) == 0) {
			args_add(stripped_args, argv[i]);
			args_add(stripped_args, "-ldl");
			continue;
		}
	...
	if (mode == MODE_COMPILE) {
		...
		if (!nodefaultlibs) {
			if (id_shared_library) {
				...
				if (cplusplus) {
					args_add_with_spaces(stripped_args, "-lstlport");
	...
	if (cplusplus) {
		if (getenv("CONFIG_LIB_STLPORT")) {
			...
			args_add_prefix(stripped_args, "-isystem");
			x_asprintf(&e, "%s/include/bits", stagedir);
			args_add_prefix(stripped_args, e);
			args_add_prefix(stripped_args, "-isystem");
			/* mark as system so that GCC doesn't complain about #include_next */
			x_asprintf(&e, "%s/include/stlport", stagedir);
			args_add_prefix(stripped_args, e);
			args_add_prefix(stripped_args, "-isystem");
		}
	...
	x_asprintf(&e, "%s/include/include-linux", stagedir);
	args_add_prefix(stripped_args, e);
	args_add_prefix(stripped_args, "-idirafter");
