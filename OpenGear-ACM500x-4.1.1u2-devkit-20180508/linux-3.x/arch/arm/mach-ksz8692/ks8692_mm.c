...
unsigned volatile int acm550x_stop_snapdog;
EXPORT_SYMBOL( acm550x_stop_snapdog );
#endif

static void ks8692_restart(char mode, const char *cmd)
{
	unsigned int val;
...
#ifdef CONFIG_MACH_ACM550X
	/* Stop snapdog from servicing any external hardware watchdog, in order
	 * to cause a reboot */
	acm550x_stop_snapdog = 1;

	/* sleep 10 seconds to let the software watchdog take over in case 
	 * the hardware watchdog jumper isn't fitted */
	msleep(10 * 1000);        
#endif
...
#else
MACHINE_START(ACM500X, "TrippLite B095")
#endif
...
#else
MACHINE_START(ACM550X, "TrippLite B094")
#endif
	/* Maintainer: OpenGear Inc */
	.atag_offset	= 0x00000100,
	.init_machine	= ks8692_init,
