...
#elif !defined(CONFIG_PCI) && !defined(KS8695X)
...
#if	!defined(CONFIG_PCI) && !defined(KS8695X)
...
static __inline void ResetDma(PADAPTER_STRUCT Adapter)
{
	...
	uRxReg &= ~(DMA_PROMISCUOUS | DMA_MULTICAST);
	...
	if (netdev->flags & (IFF_ALLMULTI
		...
		DRV_INFO("%s: enabling multicast", __FUNCTION__); 
...
static int hook_irqs(struct net_device *netdev, int req)
{
	...
	case DMA_WAN:
		if (DI.uLinkIntMask & INT_WAN_LINK) {
			if (req) {
#ifndef	USE_FIQ
				if (request_irq(31, ks8695_isr_link, IRQF_SHARED, "WAN eth", netdev)) {
#else
	...
	for (i = 0; i < 6; i++) {
		if (DI.uIntMask & (1L << (DI.uIntShift + i))) {
			if (req) {
#ifndef	USE_FIQ
				if (request_irq(i + DI.uIntShift, &ks8695_isr, IRQF_SHARED, "LAN eth", netdev)) {
#else
				if (request_irq(i + DI.uIntShift, &ks8695_isr, IRQF_SHARED | IRQF_DISABLED, "LAN eth", netdev)) {
#endif
					return -EBUSY;
				}
			}
...
void ks8695_getmac(unsigned char *dst, int index)
{
	...
	ep = ioremap(MAC_MEMBASE, 0x40000);
	if (ep) {
...
static const struct net_device_ops ks8695_netdev_ops = {
	...
	.ndo_stop		= ks8695_close,
	.ndo_set_mac_address	= ks8695_set_mac,
	.ndo_set_rx_mode	= ks8695_set_multi,
	.ndo_get_stats		= ks8695_get_stats,
	.ndo_do_ioctl		= ks8695_ioctl,
	.ndo_start_xmit		= ks8695_xmit_frame,
	.ndo_change_mtu		= ks8695_change_mtu,
	.ndo_tx_timeout		= ks8695_tx_timeout,
};
...
/* #endif */
	...
	netdev->netdev_ops = &ks8695_netdev_ops;
	...
	/* reset spinlock */
	DI.lock = __SPIN_LOCK_UNLOCKED(DI.lock);
	DI.lock_refill = __SPIN_LOCK_UNLOCKED(DI.lock_refill);
...
int ks8695_open(struct net_device *netdev)
{
	...
	netdev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
	netdev->dev.dma_mask = &netdev->dev.coherent_dma_mask;
	netdev->dev.bus = &platform_bus_type;
...
static int AllocateTxDescriptors(PADAPTER_STRUCT Adapter)
{
	...
	DI.pTxDescriptors = dma_alloc_coherent(&Adapter->netdev->dev, size, &DI.TxDescDMA, GFP_KERNEL);
...
static int AllocateRxDescriptors(PADAPTER_STRUCT Adapter)
{
	...
	DI.pRxDescriptors = dma_alloc_coherent(&Adapter->netdev->dev, size, &DI.RxDescDMA, GFP_KERNEL);
...
static void FreeTxDescriptors(PADAPTER_STRUCT Adapter)
{
	...
	size = KS8695_ROUNDUP(DI.nTxDescTotal * sizeof(TXDESC) + DESC_ALIGNMENT, BUFFER_4K);
	dma_free_coherent(&Adapter->netdev->dev, size, (void *)DI.pTxDescriptors, DI.TxDescDMA);
...
static void FreeRxDescriptors(PADAPTER_STRUCT Adapter)
{
	...
	size = KS8695_ROUNDUP(DI.nRxDescTotal * sizeof(RXDESC) + DESC_ALIGNMENT, BUFFER_4K);
	dma_free_coherent(&Adapter->netdev->dev, size, (void *)DI.pRxDescriptors, DI.RxDescDMA);
...
void ks8695_watchdog(unsigned long data)
{
	...
	if (DMA_LAN == DI.usDMAId) {
		...
		if (nCheck++ > 6) {
			...
			/*
			 * Bring the netdev link up IF ANY switch port has link.
			 * Only bring the interface down IF NO ports have link.
			 */
			ks8695_report_carrier(netdev, (linkports ? TRUE : FALSE));
		}
	}
	else {
		if (!DI.bLinkActive[0]) {
...
int ks8695_xmit_frame(struct sk_buff *skb, struct net_device *netdev)
{
	...
	DI.pTxSkb[i].dma = dma_map_single(&netdev->dev, data, DI.uRxBufferLen, PCI_DMA_TODEVICE);
...
static int __inline ProcessTxInterrupts(PADAPTER_STRUCT Adapter)
{
	...
	while (!(le32_to_cpu(TransmitDescriptor->TxOwnBit) & DESC_OWN_BIT) && DI.nTransmitCount > 0) {
		...
		dma_unmap_single(&Adapter->netdev->dev, DI.pTxSkb[i].dma, DI.uRxBufferLen, PCI_DMA_TODEVICE);
...
static int __inline ProcessRxInterrupts(PADAPTER_STRUCT Adapter)
{
	...
	while (!((uFrameCtrl = le32_to_cpu(CurrentDescriptor->RxFrameControl)) & DESC_OWN_BIT)) {
		...
		/* to do something ? */
		dma_unmap_single(&Adapter->netdev->dev, DI.pRxSkb[i].dma, DI.uRxBufferLen, PCI_DMA_FROMDEVICE);
...
static void ReceiveBufferFill(uintptr_t data)
{
	...
	while (NULL == DI.pRxSkb[i].skb) {
		...
		DI.pRxSkb[i].dma = dma_map_single(&Adapter->netdev->dev, skb->data, DI.uRxBufferLen, PCI_DMA_FROMDEVICE);
...
int ks8695_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
    ...
    switch(cmd) {
		...
		case SIOCDEVPRIVATE + 1:
			 ...
			 */
			if (sprintf(iface, "eth%d", LANPORT) && !strcmp(netdev->name, iface)) {
				if (data->phy_id >= SW_MAX_LAN_PORTS || DI.usDMAId != DMA_LAN)
					return -EOPNOTSUPP;
				regs = &mii_regs_lan[data->phy_id];
			} else if (sprintf(iface, "eth%d", WANPORT) && !strcmp(netdev->name, iface)) {
					...
					case DUMP_PCI_SPACE:
						if (pIoCtrl->usLen >= sizeof(IOCTRL)) { /* 1 + 2 + 1 */
							if (Adapter->rev) {
								...
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x2000)); i += 4;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x2004)); i += 4;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x2008)); i += 4;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x200C)); i += 4;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x2010)); i += 4;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x2014)); i += 4;
								i = 0x2c;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x202C)); i += 4;
								i = 0x3c;
								printk("0x%04x          0x%08x\n", i, KS8695_READ_REG(0x203C)); i += 4;
								nRet = 0;
							}	
						}
						break;
...
int ks8695_ioctl_switch(PADAPTER_STRUCT Adapter, PIOCTRL_SWITCH pIoCtrl)
{
    ...
    switch(pIoCtrl->bySubId) {
		...
		case CONFIG_SW_SUBID_PORT_VLAN:
			if (pIoCtrl->usLen == 20) { /* 1 + 2 + 1 + 4 * 4 */
			...
			} else if (pIoCtrl->usLen == 32) { /* 1 + 2 + 1 + 4 * 7 */
				/*
				 * Used by Opengear swtool, per port configuration
				 */
				uPort = pIoCtrl->u.uData[0];
				if (uPort > SW_MAX_LAN_PORTS) {
					break;
				...
				if (CONFIG_SWITCH_SET == pIoCtrl->byId) {
					...
					DPI[uPort].usTag = (uint16_t)pIoCtrl->u.uData[1];
					bInsertion = pIoCtrl->u.uData[2];
					bRemoval = pIoCtrl->u.uData[3];
					bFilter = pIoCtrl->u.uData[4];
					bDiscard = pIoCtrl->u.uData[5];
					DPI[uPort].byCrossTalkMask = (uint8_t)(pIoCtrl->u.uData[6] & 0x1f);

					swConfigurePort(Adapter, uPort);
					swConfigTagInsertion(Adapter, uPort, bInsertion);
					swConfigTagRemoval(Adapter, uPort, bRemoval);
					configureVID(Adapter, uPort, bFilter, bDiscard);
				} else {
					uint32_t	uReg;

					uReg = KS8695_READ_REG(KS8695_SWITCH_ADVANCED);
					pIoCtrl->u.uData[1] = (uint32_t)DPI[uPort].usTag;
					pIoCtrl->u.uData[2] = (uReg & (1L << (17 + uPort))) ? TRUE : FALSE;
					pIoCtrl->u.uData[3] = (uReg & (1L << (22 + uPort))) ? TRUE : FALSE;
					pIoCtrl->u.uData[6] = (uint32_t)DPI[uPort].byCrossTalkMask;

					uReg = KS8695_READ_REG((SW_MAX_LAN_PORTS == uPort) ? KS8695_SEP5C2 : (KS8695_SEP1C2 + uPort * 0x0c));
					pIoCtrl->u.uData[4] = (uReg & KS8695_SEPC2_VLAN_FILTER) ? TRUE : FALSE;
					pIoCtrl->u.uData[5] = (uReg & KS8695_SEPC2_DISCARD_NON_PVID) ? TRUE : FALSE;
				}
				nRet = 0;
#endif
			}
...
#ifdef CONFIG_MACH_IM4004
		case CONFIG_SW_SUBID_VLAN:
			/*
			 * Used by Opengear swtool, VLAN configuration
			 */
			if (pIoCtrl->usLen == 8) { /* 1 + 2 + 1 + 4 * 1 */
				uint32_t	uReg;
				uint32_t	vlan_enable;

				if (CONFIG_SWITCH_SET == pIoCtrl->byId) {
					vlan_enable = pIoCtrl->u.uData[0];
					...
					uReg = KS8695_READ_REG(KS8695_SEC1);
					pIoCtrl->u.uData[0] = (uReg & KS8695_SEC1_8021Q_VLAN_EN) ? TRUE : FALSE;
				}
				nRet = 0;
			} else if (pIoCtrl->usLen == 24) { /* 1 + 2 + 1 + 4 * 5 */
				uint32_t	index;
				
				index = pIoCtrl->u.uData[0];
				if (index > 15) {
					break;
				}
				if (CONFIG_SWITCH_SET == pIoCtrl->byId) {
					...
					valid = pIoCtrl->u.uData[1];
					members = pIoCtrl->u.uData[2];
					fid = pIoCtrl->u.uData[3];
					vid = pIoCtrl->u.uData[4];
	
					uReg = valid ? 1L << 21 : 0;
					uReg |= members << 16;
					uReg |= fid << 12;
					uReg |= vid;
					KS8695_WRITE_REG(KS8695_SEIADL, uReg);
				        DelayInMicroseconds(10);

					/* Trigger the write from indirect access register */
					KS8695_WRITE_REG(KS8695_SEIAC, KS8695_SEIAC_WRITE | KS8695_SEIAC_TAB_VLAN | index);
			        DelayInMicroseconds(10);
				} else {
					/* Trigger the write to indirect access register */
					KS8695_WRITE_REG(KS8695_SEIAC, KS8695_SEIAC_READ | KS8695_SEIAC_TAB_VLAN | index);
				        DelayInMicroseconds(10);

					uReg = KS8695_READ_REG(KS8695_SEIADL);
					pIoCtrl->u.uData[1] = (uReg & 1L << 21) ? TRUE : FALSE;
					pIoCtrl->u.uData[2] = (uReg >> 16) & 0x1F;
					pIoCtrl->u.uData[3] = (uReg >> 12) & 0xF;
					pIoCtrl->u.uData[4] = uReg & 0xFFF;
				}
				nRet = 0;
			}
			break;
