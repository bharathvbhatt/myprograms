...
#ifdef CONFIG_LEDMAN
			ledman_cmd(LEDMAN_CMD_SET, (ndev->name[3] == '0') ?
...
/**
 *	ks8695_lan_get_settings - Get device-specific settings.
 *	@ndev: The network device to read settings from
 *	@cmd: The ethtool structure to read into
 */
static int
ks8695_lan_get_settings(struct net_device *ndev, struct ethtool_cmd *cmd)
{
	struct ks8695_priv *ksp = netdev_priv(ndev);
	u32 ctrl;
	...
	ctrl = readl(ksp->phyiface_regs + KS8695_SEPXCZ(1,1));
	if (!(ctrl & 1 << 15)) {
		ctrl = readl(ksp->phyiface_regs + KS8695_SEP12AN);
		/* auto-negotiation is enabled */
		cmd->advertising |= ADVERTISED_Autoneg;
		
		if (ctrl & (SEP_AN_AD_100FULL << PORT_SHIFT))
			cmd->advertising |= ADVERTISED_100baseT_Full;
		if (ctrl & (SEP_AN_AD_100HALF << PORT_SHIFT))
			cmd->advertising |= ADVERTISED_100baseT_Half;
		if (ctrl & (SEP_AN_AD_10FULL << PORT_SHIFT))
			cmd->advertising |= ADVERTISED_10baseT_Full;
		if (ctrl & (SEP_AN_AD_10HALF << PORT_SHIFT))
			cmd->advertising |= ADVERTISED_10baseT_Half;
		if (ctrl & (SEP_AN_AD_FLOW << PORT_SHIFT)) {	
			cmd->advertising |= ADVERTISED_Pause;
		}
		cmd->autoneg = AUTONEG_ENABLE;

		ethtool_cmd_speed_set(cmd,
				      (ctrl & (SEP_AN_LINK_SPEED_STATUS << PORT_SHIFT)) ? SPEED_100 : SPEED_10);
		cmd->duplex = (ctrl & (SEP_AN_LINK_DUPLEX_STATUS << PORT_SHIFT)) ?
			DUPLEX_FULL : DUPLEX_HALF;
	} else {
		/* auto-negotiation is disabled */
		cmd->autoneg = AUTONEG_DISABLE;

		ethtool_cmd_speed_set(cmd, ((ctrl & (1 << 14)) ?
					    SPEED_100 : SPEED_10));
		cmd->duplex = (ctrl & (1 << 13)) ?
			DUPLEX_FULL : DUPLEX_HALF;
	}
...
/**
 *	ks8695_lan_set_settings - Set device-specific settings.
 *	@ndev: The network device to configure
 *	@cmd: The settings to configure
 */
static int
ks8695_lan_set_settings(struct net_device *ndev, struct ethtool_cmd *cmd)
{
	struct ks8695_priv *ksp = netdev_priv(ndev);
	u32 ctrl;
	...
	if (cmd->autoneg == AUTONEG_ENABLE) {
		...
		ctrl = readl(ksp->phyiface_regs + KS8695_SEPXCZ(1,1));
		/* Clear the disable autoneg bit */
		ctrl &= ~(1 << 15);
		writel(ctrl, ksp->phyiface_regs + KS8695_SEPXCZ(1,1));
		
		ctrl = readl(ksp->phyiface_regs + KS8695_SEP12AN);

		ctrl &= ~(
				(SEP_AN_AD_100FULL << PORT_SHIFT)
				|
				(SEP_AN_AD_100HALF << PORT_SHIFT)
				|
				(SEP_AN_AD_10FULL << PORT_SHIFT)
				|
				(SEP_AN_AD_10HALF << PORT_SHIFT)	
			);
		if (cmd->advertising & ADVERTISED_100baseT_Full)
			ctrl |= (SEP_AN_AD_100FULL << PORT_SHIFT);
		if (cmd->advertising & ADVERTISED_100baseT_Half)
			ctrl |= (SEP_AN_AD_100HALF << PORT_SHIFT);
		if (cmd->advertising & ADVERTISED_10baseT_Full)
			ctrl |= (SEP_AN_AD_10FULL << PORT_SHIFT);
		if (cmd->advertising & ADVERTISED_10baseT_Half)
			ctrl |= (SEP_AN_AD_10HALF << PORT_SHIFT);

		/* force a re-negotiation */
		ctrl |= (SEP_AN_RESTART_AN << PORT_SHIFT);
		writel(ctrl, ksp->phyiface_regs + KS8695_SEP12AN);
	} else {
		ctrl = readl(ksp->phyiface_regs + KS8695_SEPXCZ(1,1));
		/* disable auto-negotiation */
		ctrl |= (1 << 15);
...
/**
 *	ks8695_lan_nwayreset - Restart the autonegotiation on the port.
 *	@ndev: The network device to restart autoneotiation on
 */
static int
ks8695_lan_nwayreset(struct net_device *ndev)
{
	struct ks8695_priv *ksp = netdev_priv(ndev);
	u32 ctrl;
	...
	if ((ctrl & 1 << 15) == 0) {
		ctrl = readl(ksp->phyiface_regs + KS8695_SEP12AN);
		writel(ctrl | (SEP_AN_RESTART_AN << PORT_SHIFT),
		       ksp->phyiface_regs + KS8695_SEP12AN);
	} else {
		/* auto-negotiation not enabled */
		return -EINVAL;
	}
...
/**
 *	ks8695_lan_get_pause - Retrieve network pause/flow-control advertising
 *	@ndev: The device to retrieve settings from
 *	@param: The structure to fill out with the information
 */
static void
ks8695_lan_get_pause(struct net_device *ndev, struct ethtool_pauseparam *param)
{
	struct ks8695_priv *ksp = netdev_priv(ndev);
	u32 ctrl;
	...
	/* advertise Pause */
	param->autoneg = (ctrl & (SEP_AN_AD_FLOW << PORT_SHIFT));
...
static u32
ks8695_lan_get_link(struct net_device *ndev) 
{
	struct ks8695_priv *ksp = netdev_priv(ndev);
	u32 ctrl;

	ctrl = readl(ksp->phyiface_regs + KS8695_SEP12AN);
	return (ctrl & (SEP_AN_LINK_STATUS << PORT_SHIFT) ? 1 : 0);	
}
/**
...
static const struct ethtool_ops ks8695_ethtool_ops = {
	...
	.get_settings	= ks8695_lan_get_settings,
	.set_settings	= ks8695_lan_set_settings,
	.nway_reset	= ks8695_lan_nwayreset,
	.get_link	= ks8695_lan_get_link,
	.get_pauseparam = ks8695_lan_get_pause,
...
ks8695_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	ledman_cmd(LEDMAN_CMD_SET,
		(ndev->name[3] == '0') ? LEDMAN_LAN1_TX : LEDMAN_LAN2_TX);
#endif
...
static int ks8695_ioctl(struct net_device *netdev, struct ifreq *rq, int cmd) 
{
	struct ks8695_priv *ksp;
	int *data;
	data = (int *) & rq->ifr_ifru;
	ksp = netdev_priv(netdev);

	switch (cmd) {
		/* Switch Port 1 Link Status - Data is used as output*/
		case SIOCDEVPRIVATE: 
		{
			u32 sep12an;	
			if (ksp->dtype != KS8695_DTYPE_LAN) {
				dev_err(ksp->dev, "Switch IOCTL run on WAN port\n");
				return -EINVAL;
			}
			sep12an = readl(ksp->phyiface_regs + KS8695_SEP12AN);
			/* Port 1 link status is bit 23 */
			if (sep12an & (1 << 23)) {
				data[0] = 1;
			} else {
				data[0] = 0;
		...
		break;
		/* Switch Disable/Enable - Data is used as input*/
		case SIOCDEVPRIVATE+1:
		{
			...
			sec0 = readl(ksp->phyiface_regs + KS8695_SEC0);
			if (data[0] == 1) {
				sec0 |= SEC0_ENABLE; 	
			} else {
				sec0 &= ~SEC0_ENABLE;
			}
			writel(sec0, ksp->phyiface_regs + KS8695_SEC0);
		}
		break;
	}
