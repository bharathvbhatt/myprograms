    ...
    PUSHORT   pwData )
{
    ...
    /* We need to get our Phy into the valid Phy registers */
#ifdef CONFIG_MACH_ACM550X
    int oldreg;
    oldreg = KS_READ_REG(KS8692_STA_PHY);
    KS_WRITE_REG(KS8692_STA_PHY, STA_PHY0_VALID | port);
#endif
...
#ifdef CONFIG_MACH_ACM550X
    KS_WRITE_REG(KS8692_STA_PHY, oldreg);
#endif
    ...
    PHARDWARE pHardware )
{
    ...
    pHardware->m_bLinkIntWorking = FALSE;
#endif
    ...
    PHARDWARE pHardware )
{
    ...
    /* Vendor supplied code here dropped to 125MHz, but additional
     * code found at
     * http://www.micrel.com/_PDF/Ethernet/sw_designkit/8692P/CUST/ks8692-linux2.6.23-20091015.bz2
     * shows their u-boot code dropping the bus clock back to 50MHz. In testing
     * this seems to prevent the system hang more reliably that 125MHz */
    KS_WRITE_REG( KS8692_SYSTEM_BUS_CLOCK,
        ( data & ~SYSTEM_BUS_CLOCK_MASK ) | SYSTEM_BUS_CLOCK_50 );

    /* Also found that a delay here helps avoid the system hang */
    mdelay(1);
    DBG_PRINT("ks8692: resetting dma controller\n");
#endif
