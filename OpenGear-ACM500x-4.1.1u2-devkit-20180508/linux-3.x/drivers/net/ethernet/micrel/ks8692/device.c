    ...
    struct dev_info* pAdapter )
{
    ...
    if ( pHardware->m_TxIntMask > 4 )
    {
        pHardware->m_TxIntMask = 4;
    }
    ...
    unsigned long ptr )
{
	...
	if (pHardware->m_hasSwitch == 1) {
		/* our link is always up */
	...
	/* Only check link if we don't have a switch - otherwise we stay up */
	if (pHardware->m_hasSwitch != 1)	
#endif 
		{
			HardwareGetLinkSpeed( pHardware );
...
#ifdef ALIGN_IP_HEADER
    /* new_mtu + header + crc(4) + vlan tag(4) + reserved , rounded up */
    rx_size = ( new_mtu + ETHERNET_HEADER_SIZE + 4 + 4 + reserved + 3 ) & ~3;
#ifdef CONFIG_KSZ8692VA
...
/* RX Watchdog - reset stuff if we're dropping packets, but not getting RX interrupts */

static void rx_checker(unsigned long data)
{
	struct net_device* dev = ( struct net_device* ) data;
	struct dev_priv* priv = netdev_priv(dev);
	struct dev_info* hw_priv = priv->pDevInfo;
	PHARDWARE        pHardware = &hw_priv->hw;
	/* Check the drop counter for this netdev */
	u32 reg = 0;
	
	if (dev->mem_start == dev->base_addr + KS_WAN_DMA_TX) {
		/*eth1 */
		reg = KS8692_READ_REG(KS8692_WAN_PKT_DROP);
	} else {
		reg = KS8692_READ_REG(KS8692_LAN_PKT_DROP); 
	}
	if (reg > 0) {
		if (pHardware->m_rxIntrCount == 0) {
			DEV_TRANSMIT_TIMEOUT(dev);	
		} else {
			pHardware->m_rxIntrCount = 0;
		}
	}	
	mod_timer(&pHardware->m_rxChecker, jiffies + HZ);
}
/*
    netdev_init
    ...
    struct net_device* dev )
{
    ...
    /* Invert port ordering, we want LAN on eth0, WAN on eth1 */
    if (!device_present)
        wanport = 0;
#else
    if (device_present)
    ...
    if ( !last_dev ) {
		...
		/* Don't trigger initially */
		pHardware->m_rxIntrCount = 1;
		init_timer(&pHardware->m_rxChecker);
		pHardware->m_rxChecker.function = &rx_checker;
		pHardware->m_rxChecker.data = (unsigned long)dev;
		mod_timer(&pHardware->m_rxChecker, jiffies + HZ);
...
#ifdef ALIGN_IP_HEADER
        /* ethernet + crc(4) + vlan tag(4), rounded up */
        hw_priv->m_nMTU = ( MAXIMUM_ETHERNET_PACKET_SIZE + 4 + 4 + 3 ) & ~3;
...
#ifdef CONFIG_MACH_ACM550X
	if (wanport) {
		/* We need to probe to see if we've got a switch or a normal phy */
		u16 phy_model;
		/* We start off looking for the PHY id in the phy
		 * We can do this on either the normal Phy, or the switch 
		 */
		HardwareReadPhy(1, 3, &phy_model);
		if (phy_model == 0x1450) {
			/* Switch Found */
			Dummy_InitPhy();
			hw_priv->hw.m_hasSwitch = 1;
			hw_priv->hw.m_ulHardwareState = MediaStateConnected;
		} else {
			hw_priv->hw.m_hasSwitch = 0;
...
#ifdef CONFIG_MACH_ACM550X
		// Always need to re-init the phy''s here on the ACM550x due
		// to the fact that dummy phy has overwritten the defaults
		if ( ks8001_InitPhy() )
			;
...
#if defined(CONFIG_MACH_ACM550X) || defined(CONFIG_MACH_ACM500X)
/* We want to poll link status in the dev_monitor task */
	return IRQ_HANDLED;
...
#if defined(CONFIG_MACH_ACM500X) || defined(CONFIG_MACH_ACM550X)
	/* Invert the order of the ports, port 0 is LAN, port 1 WAN */
	pdev->resource = &net_resources[ i ^ 0x1];
#else
...
/*
 *	The external PHY's will not be accessible unless we set their
 *	addresses into the KS8692_STA_PHY register. Maybe these could
 *	go in platform code, but I think I will find them easier in the
 *	future if I put them here in the driver code proper.
 */
#define	PHYADDR0	1
#define	PHYADDR1	5
	KS_WRITE_REG( KS8692_STA_PHY,
		STA_PHY1_VALID | (PHYADDR1 << 8) |
		STA_PHY0_VALID | PHYADDR0 );
#endif
