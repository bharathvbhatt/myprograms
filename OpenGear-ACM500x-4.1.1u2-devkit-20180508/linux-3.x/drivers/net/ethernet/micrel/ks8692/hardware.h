...
typedef struct
{
    ...
    /* 
     * ACM5504-5s have a 4 port switch on eth1 
     * We ignore mdio interrupts from this port when this is set to 
     * 1, and make sure we call the hardware link status function, which
     * correctly checks the link status registers for the particular ports
     */ 
    int 		    m_hasSwitch;
#endif
} HARDWARE, *PHARDWARE;
