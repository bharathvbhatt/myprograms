/* linux/drivers/i2c/busses/i2c-ks8692.c
 *
...
#define DEFAULT_I2C_BIT_RATE    100000                     /* 100KHz bit rate */
...
static inline int ks8692_i2c_wait_SCL ( struct ks8692_i2c *i2c )
{
    ...
    while (!(iic_sr & I2C_SCL) || !(iic_sr & I2C_SDA)) {
        if (signal_pending(current)) {
            ...
            printk("ks8692_i2c_wait_SCL(): Interrupted. iic_sr 0x%08X: %08x\n", 
                      (int)(i2c->regs + KS8692_I2C_STATUS), (int)iic_sr);
            return -EINTR;
        }
        if (time_after(jiffies, orig_jiffies + HZ)) {
            pr_debug("I2C: timeout\n");
            printk("ks8692_i2c_wait_SCL(): timeout. iic_sr 0x%08X: %08x\n", 
                      (int)(i2c->regs + KS8692_I2C_STATUS), (int)iic_sr);
            return -EIO;
        }
        schedule();
...
static int i2c_wait(struct ks8692_i2c *i2c, unsigned timeout, int writing)
{
    ...
    if (i2c->irq == 0) {
        ...
        result = wait_event_interruptible_timeout(i2c->wait,
			                       i2c->i2c_done, timeout);

        DBG_PRINT("iic_sr 0x%08X: %08x, i2c_done=%d, result=0x%x\n", 
                   (int)(i2c->regs + KS8692_I2C_STATUS), (int)i2c->i2c_status, (int)i2c->i2c_done, (int)result);

        if (unlikely(result < 0)) {
            pr_debug("I2C: wait interrupted\n");
            printk("i2c_wait: wait interrupted. result %d\n", result);
        }
	...
	if ( !(i2c->i2c_status & I2C_DATA_DONE) || (i2c->i2c_status & I2C_ERROR_MASK) ) {
	...
	DBG_PRINT("%s transaction is completed by %d bytes. (%08x)\n",
               writing ? "write" : "read" , (int)ret_length, (int)i2c->i2c_status);

	return (ret_length);
}

static inline int ks8692_i2c_start
(
...
static irqreturn_t ks8692_i2c_irq(int irqno, void *dev_id)
{
    ...
    if ( isr & INT_I2C ) {
	    ...
	    if ( !(iic_scr & I2C_START)) {
            ...
            /* Record I2C transaction status to clear interrupt source */
	        i2c->i2c_status = readl(i2c->regs + KS8692_I2C_STATUS);

            DBG_PRINT("iic_sr 0x%08X: %08x\n", 
                       (int)(i2c->regs + KS8692_I2C_STATUS), (int)i2c->i2c_status);

		    wake_up_interruptible(&i2c->wait);
	    }

        /* Ack I2C interrupt */
	    //KS8692_WRITE_REG(KS8692_INT_ENABLE2, isr | INT_I2C);
    }
    else {
	    iic_scr = readl(i2c->regs + KS8692_I2C_COMMAND);
        printk("ks8692_i2c_irq(): unexpect interrupt. iic_sr 0x%08X: %08x\n", 
                      (int)(i2c->regs + KS8692_I2C_COMMAND), (int)iic_scr);
    }
 ...
 */
static inline void ks8692_i2c_prepare_for_read
(
    struct ks8692_i2c *i2c, 
 ...
 */
static inline void ks8692_i2c_prepare_for_write
(
    struct ks8692_i2c *i2c, 
    unsigned int    dev_addr,
    int  write_length 
 ...
 */
static int ks8692_i2c_byte_write
(
    struct ks8692_i2c *i2c, 
    struct i2c_msg *msg
)
{
    ...
    int    actuallyWriteLen;
    unsigned long regData=0;
    int i;

    DBG_PRINT("dev_addr=%x, data=%08x, length=%d, timeout=%d\n", 
              (int)dev_addr, (int)data, (int)length, (int)timeout);
    ...
    /* Some data to write */
    while ( length > 0 ) {
           writeLen = length > 8 ? 8 : length;

           /* Configure I2C controller, prepare I2C write operation */
           ks8692_i2c_prepare_for_write (i2c, dev_addr, writeLen);

           /* Write to I2C_WDR0 */
           writeLenTemp = length > 4 ? 4 : length;
	   regData = 0;
	   for (i=0; i < writeLenTemp; i++)
		regData |= (*data++ << (i*8));
	   ...
	   length -= writeLenTemp;
		...
		for (i=0; i < writeLenTemp; i++)
		    regData |= (*data++ << (i*8));
		writel(regData, i2c->regs + KS8692_I2C_WRITE_DATA1);
		DBG_PRINT("I2C_WDR1 0x%08X: %08x\n", 
                    (int)(i2c->regs + KS8692_I2C_WRITE_DATA1), 
                    (int)readl(i2c->regs + KS8692_I2C_WRITE_DATA1));
		length -= writeLenTemp;
           }
 ...
 */
static int ks8692_i2c_random_read
(
    struct ks8692_i2c *i2c, 
    struct i2c_msg *msg
)
{
    ...
    while ( length > 0 ) {
           readLen = length > 8 ? 8 : length;

           /*
            * Transfer I2C device address to I2C bus with READ operation.
            * (Initiates a current address read).
            */
           ks8692_i2c_prepare_for_read (i2c, dev_addr, readLen);

           /* Start I2C transaction */
           if ( ks8692_i2c_start (i2c, timeout, 0, &actuallyReadLen) == -1 )
              return -1 ;
           ...
           DBG_PRINT("I2C_RDR0 0x%08X: %08x\n", 
                 (int)(i2c->regs + KS8692_I2C_READ_DATA0), (int)regData);
           ...
           readLen -= readLenTemp;
           if (readLen > 0) {
               regData = readb(i2c->regs + KS8692_I2C_READ_DATA1);
               bData = (unsigned char *)&regData;
               DBG_PRINT("I2C_RDR1 0x%08X: %08x\n", 
                     (int)(i2c->regs + KS8692_I2C_READ_DATA1), (int)regData);
...
)
{
    ...
    for (i = 0; ret >= 0 && i < num; i++) {
        ...
        if (pmsg->flags & I2C_M_RD)
            ret = ks8692_i2c_random_read(i2c, pmsg);
        else
            ret = ks8692_i2c_byte_write(i2c, pmsg);
    }
...
static int ks8692_i2c_setclock(struct ks8692_i2c *i2c)
{
    ...
    DBG_PRINT("sysclk=%d, bit_rate=%d, bitPeriod=%d(0x%x)\n", 
        (int)i2c->sysclk, (int)i2c->bit_rate, (int)bitPeriod, (int)bitPeriod );
    ...
    DBG_PRINT("%08X: %08x\n", (int)(i2c->regs + KS8692_I2C_CTRL), (int)regData);
 ...
 */
static struct resource *platform_device_resource(struct platform_device *pdev, unsigned int mask, int nr)
{
    int i;
...
static int ks8692_i2c_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: Micrel Pegasus I2C master controller\n", dev_name(&i2c->adap.dev));
