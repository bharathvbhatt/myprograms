 ...
 * compatible with it.
 */
static u32 kirkwood_fix_sscg_deviation(u32 system_clk)
{
				 ...
				 struct core_clocks *coreclk)
{
	...
	if (coreclk->is_sscg_enabled && coreclk->fix_sscg_deviation
			&& coreclk->is_sscg_enabled(base))
		rate = coreclk->fix_sscg_deviation(rate);
