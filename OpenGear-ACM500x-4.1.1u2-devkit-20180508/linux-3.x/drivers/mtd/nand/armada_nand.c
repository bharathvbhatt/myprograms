...
#ifdef CONFIG_MTD_NAND_ARMADA_SOFT_BCH
	}, { 	/* No ECC */
		NULL, &mv_lp_bb, 8102, 2048, 0, 4, 0, 744
#else
	}, { 	/* No ECC */
		NULL, NULL, 0, 0, 0, 0, 0, 0
#endif
...
#define NFC_FLAGS_SWECC_OVERRIDE	0x4
...
MV_NFC_FLASH_INFO flashDeviceInfo[] = {
	...
	.flags = (NFC_CLOCK_UPSCALE_200M | NFC_FLAGS_ONFI_MODE_3_SET)
        ...
        .tRH = 40,             /* tRH, ND_nRE high duration */
        ...
        .pgPrBlk = 256,        /* Pages per block - detected */
        .pgSz = 8192,          /* Page size */
        .oobSz = 774,          /* Spare size */
        .blkNum = 2128,        /* Number of blocks/sectors in the flash */
        .id = 0x442C,          /* Device ID 0xDevice,Vendor */
        .model = "Micron 32Gb 8bit",
        .bb_page = 0,          /* Manufacturer Bad block marking page in block */
        .flags = (NFC_CLOCK_UPSCALE_200M | NFC_FLAGS_SWECC_OVERRIDE)
        },
};
...
#ifdef CONFIG_MTD_NAND_ARMADA_SOFT_BCH
         /* Override the default ECC setting to Soft BCH if we need to */
         if (flashDeviceInfo[i].flags & NFC_FLAGS_SWECC_OVERRIDE) {
                 nfcInfo->eccMode = MV_NFC_ECC_DISABLE;
                 nfcCtrl->sweccOverride = MV_TRUE;
                 /* Re-enable the spare area */
                 ctrl_reg |= NFC_CTRL_SPARE_EN_MASK;
                 MV_REG_WRITE(NFC_CONTROL_REG, ctrl_reg);
         }
#endif
...
static int orion_nfc_do_cmd_pio(struct orion_nfc_info *info)
{
	...
	for (i=0; i < numCmds; i++) {
		...
		if (ndcr & NFC_CTRL_ND_RUN_MASK) {
			/* Command underflow case */
			if ((info->dscr & NFC_SR_WRCMDREQ_MASK) == 0) {
				printk(KERN_DEBUG "WRONG NFC STAUS: command %d, NDCR=0x%08x, NDSR=0x%08x(0x%08x), NDECCCTRL=0x%08x)\n",
				info->cmd, ndcr, info->dscr, MV_REG_READ(NFC_STATUS_REG), MV_REG_READ(NFC_ECC_CONTROL_REG));
			}
			MV_REG_WRITE(NFC_CONTROL_REG, (ndcr & ~NFC_CTRL_ND_RUN_MASK));
		}
	}
...
static int orion_nfc_setup_read_retry(struct mtd_info *mtd, int retry_mode)
{
	uint32_t setting = retry_mode;
	struct orion_nfc_info *info = (struct orion_nfc_info *)((struct nand_chip *)mtd->priv)->priv;

	int status = mvNfcDeviceFeatureSet(&info->nfcCtrl, NAND_CMD_SET_FEATURES, ONFI_FEATURE_ADDR_READ_RETRY, setting, 0);
	if (status == MV_FAIL) {
		return -EIO;
	}
	return 0;
...
#ifdef CONFIG_MTD_NAND_ARMADA_SOFT_BCH
	/* Check to see if we've got a software override flag */
	if (info->nfcCtrl.sweccOverride) {
		printk(KERN_ERR " Overriding HW ECC to use soft BCH\n");
		info->ecc_type          = MV_NFC_ECC_DISABLE;
		/* Force a recalculation */
		nand->oobsize_ovrd      = ((CHUNK_SPR * CHUNK_CNT) + LST_CHUNK_SPR);
		nand->ecc.mode          = NAND_ECC_SOFT_BCH;
		/* Settings for 40 bit BCH on 1024 byte chunks */
		nand->ecc.size          = 1024;
		nand->ecc.strength      = 40;
		nand->ecc.bytes         = 70;
		/* hack: this is a chip/vendor-specific setting that shouldn't be here */
		nand->read_retries      = 8;
		nand->setup_read_retry  = orion_nfc_setup_read_retry;
	} else {
...
#ifndef CONFIG_MTD_NAND_ARMADA_SOFT_BCH
