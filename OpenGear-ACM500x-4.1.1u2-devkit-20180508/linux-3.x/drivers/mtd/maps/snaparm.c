...
#ifdef CONFIG_MTD_SNAPARM_BANK
#define FLASH_ADDR 0x03800000		/* At 56-60MB region */
#define FLASH_BANK 0x00400000		/* 4MB bank size */
#define	FLASH_MAP  FLASH_BANK
#else
#define FLASH_ADDR 0x03000000
#define	FLASH_MAP  0x00fe0000		/* 16MB - 128k */
#endif
#define FLASH_SIZE 0x01000000
#define FLASH_WIDTH 1

#elif defined(CONFIG_MACH_ACM500X) || defined(CONFIG_MACH_ACM550X)

#ifdef CONFIG_MACH_ACM500X
#define	FLASH_ADDR 0x18000000
#else
#define	FLASH_ADDR 0x19000000
#endif
#define	FLASH_SIZE 0x02000000
#define	FLASH_WIDTH 1
...
#define	FLASH_MAP	(sg_map.size)

#endif

#ifndef FLASH_MAP
#define	FLASH_MAP	FLASH_SIZE
#endif
...
#if defined(CONFIG_MTD_SNAPARM_BANK)
/*
 * 	To make available as much RAM as possible we use a bank selected
 * 	mechanism to access all of the 16MB of FLASH. We map in a single
 * 	4MB region, as required. (4MB is the largest chip select region
 * 	size, and we need to be able to drive all its addresses lines
 * 	for complete access - so this is minumum address space that we can
 * 	bank select on).
 */
static int sg_currbank = -1;

static __init void sg_initbanks(void)
{
	u32 v;
...
static unsigned int sg_bankbits[] = {
	[0] = 0x00000001,
	[1] = 0x00000004,
	[2] = 0x00010000,
	[3] = 0x00040000,
...
static void sg_selectbank(unsigned int addr)
{
	int bank = (addr & 0x00c00000) >> 22;
	u32 v;

	if (sg_currbank != bank) {
		v = __raw_readl(KS8695_MEM_VA + KS8695_ERGCON);
		v = (v & ~0x000f000f) | sg_bankbits[bank];
		__raw_writel(v, KS8695_MEM_VA + KS8695_ERGCON);
		sg_currbank = bank;
	}
}

static inline unsigned int sg_bankofs(unsigned int addr)
{
	return (addr & 0x003fffff);
}

static inline unsigned int sg_bank2end(unsigned int addr, unsigned int len)
{
	unsigned int banklen = FLASH_BANK - addr;
	return (len < banklen) ? len : banklen;
}
...
#if !defined(CONFIG_MTD_SNAPARM_BANK)
#define sg_selectbank(a)
#define sg_bankofs(a) (a)
#define sg_bank2end(a,l) (l)
#endif
...
copybank:
	...
	sg_selectbank(from);
	ofs = sg_bankofs(from);
	thisbanklen = blen = sg_bank2end(ofs, len);
	p8 = (map->virt + ofs);
	dp = (__u8 *) to;
	 ...
	 */
	while (blen > 0 && (((unsigned long) p8) & 3)) {
		readpreflush(p8);
		*dp++ = __raw_readb(p8);
		readpostflush(p8);
		p8++;
...
copybank:
	...
	if (map_bankwidth(map) == 1) {
		u8 *src8;
		for (src8 = (u8 *) from, i = 0; (i < blen); i++) {
			writepreflush(map->virt + ofs + i);
			__raw_writeb(*src8++, map->virt + ofs + i);
			writepostflush(map->virt + ofs + i);
		}
	} else {
		u16 *src16;
		for (src16 = (u16 *) from, i = 0; (i < blen); i += 2) {
			writepreflush(map->virt + ofs + i);
			__raw_writew(*src16++, map->virt + ofs + i);
			writepostflush(map->virt + ofs + i);
		}
	}
...
/****************************************************************************/
    ...
    defined(CONFIG_MACH_ACM550X)

#ifndef CONFIG_TL_REBRAND
#define	VENDOR	"OpenGear"
#else
#define VENDOR  "TrippLite"
#endif
...
static struct mtd_partition sg_partitions[] = {
    ...
    defined(CONFIG_MACH_ACM500X) || defined(CONFIG_MACH_ACM550X)
	{
		.name	= VENDOR " system configuration",
		.offset	= BOOTSIZE,
		.size	= BOOTCFGSIZE,
	},
#endif
	{
		.name	= VENDOR " non-volatile configuration",
...
#if defined(CONFIG_ARCH_IXP4XX)
{
	...
	*IXP4XX_EXP_CS0 = val;
	...
	sg_res = request_mem_region(FLASH_ADDR, FLASH_MAP, VENDOR " FLASH");
	if (sg_res == NULL) {
	 ...
	 */
	sg_map.virt = ioremap(FLASH_ADDR, FLASH_MAP);
	if (!sg_map.virt) {
		release_mem_region(FLASH_ADDR, FLASH_MAP);
		sg_res = NULL;
	...
	/*
	 * We don't want the MTD layer directly accessing the FLASH.
	 * It might be bank switched, or it might need interleaving to
	 * empty the bus buffers.
	 */
	sg_mtd->_point = NULL;
	sg_mtd->_unpoint = NULL;
...
#ifndef CONFIG_MTD_SNAPARM_BANK
#if (defined(CONFIG_MACH_CM41xx) || defined(CONFIG_MACH_IM42xx) || \
     defined(CONFIG_MACH_IM4004)) && !defined(CONFIG_MACH_OLDCM41xx)
	/*
	 *	We are really squeezed for memory mapping space on the
	 *	CM41xx, so we map the flash such that the periphal space
	 *	is actually in the last flash segment. Adjust the size
	 *	of the device here to remove that last segment.
	 */
	if (sg_mtd->size == FLASH_SIZE) {
		sg_partitions[P_ALLFLASH].size = FLASH_SIZE - 128*1024;
		sg_partitions[P_IMAGE].size = FLASH_SIZE - sg_partitions[P_IMAGE].offset - 128*1024;
	}
#endif
#endif /* !CONFIG_MTD_SNAPARM_BANK */
