...
#define XRM117XX_IOCONTROL_GPIO_BIT	(1 << 1) /* GPIO[7:4] as modem I/O */
...
static void xrm117x_handle_msi(struct xrm117x_ch *ch, unsigned int msr)
{
	...
	if (msr & UART_MSR_DDCD)
		uart_handle_dcd_change(&ch->port, msr & UART_MSR_DCD);
	if (msr & UART_MSR_DCTS)
		uart_handle_cts_change(&ch->port, msr & UART_MSR_CTS);
...
static void xrm117x_md_proc(struct work_struct *ws)
{
	...
	unsigned int mctrl = ch->md_mctrl;
	unsigned char mcr = 0;
	if (mctrl & TIOCM_RTS)
		mcr |= UART_MCR_RTS;
	if (mctrl & TIOCM_DTR)
				  ...
				  struct ktermios *old)
{
...
#if 0 /* Higher layers will enable CTS/RTS via mctrl */
	if (termios->c_cflag & CRTSCTS)
	{
	...
	/* Configure GPIO[7:4] pins as modem control lines. */
#ifdef CONFIG_GPIOLIB
	if (!ch->xr->gpio.nr_gpio)
#endif
	{
		/* Configure DTR(GPIO5) as an output (high), and leave
		   as inputs RI(GPIO7), CD(GPIO6), DSR(GPIO4) and GPIO[3:0].
		   Otherwise, DTR will float, regardless of IOControl[1]. */
		sc_serial_out(ch, SC_IOSTATE_W, 1<<5);
		sc_serial_out(ch, SC_IODIR, 1<<5);
		sc_serial_modify(ch, SC_IOCONTROL, XRM117XX_IOCONTROL_GPIO_BIT,
			XRM117XX_IOCONTROL_GPIO_BIT);
	}
...
static int xrm117x_startup(struct uart_port *port)
{
	...
	/* Enable RX, TX, CTS and modem status change interrupts */
	val = XRM117XX_IER_RDI_BIT | XRM117XX_IER_THRI_BIT;
	val |= XRM117XX_IER_CTSI_BIT;
	val |= XRM117XX_IER_MSI_BIT;
...
#ifdef USE_SPI_MODE
/* Simple test read and write functions used for the probe */
static int test_spi_read(struct spi_device *spi, int reg, int *value) {
		u8 cmd;
		int status;
		cmd = (0x80 | (reg<<3));
		status = spi_write_then_read(spi, &cmd, 1, value, 1);

		return status;
}

static int test_spi_write(struct spi_device *spi, int reg, int value) {
	unsigned char spi_buf[2];
	int status;
	spi_buf[0] = (reg<<3);
	spi_buf[1] =  (unsigned char)value;
	status = spi_write(spi, spi_buf, 2);
	return status;
}
...
static int xrm117x_spi_probe(struct spi_device *spi)
{
	...
	/* User some of the UART existence logic tests from 8250_core to
	 * check if the device is present. If not, return -ENODEV
	 */
	{
		int status;
		int scratch, scratch2, scratch3;

		/* Check that writing to and reading from the IER reg works */
		status = test_spi_read(spi, SC_RHR, &scratch);
		if (status)
			return -ENODEV;

		status = test_spi_write(spi, SC_RHR, 0);
		if (status)
			return -ENODEV;
		/*
		...
		status = test_spi_read(spi, SC_RHR, &scratch2);
		if (status)
			return -ENODEV;

		scratch2 &= 0x0f;

		status = test_spi_write(spi, SC_RHR, 0x0f);
		if (status)
			return -ENODEV;

		status = test_spi_read(spi, SC_RHR, &scratch3);
		if (status)
			return -ENODEV;
