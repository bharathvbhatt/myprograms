...
#ifdef CONFIG_EXAR_GPIO_SUPPORT
...
static int pci_xr17v35x_setup(struct serial_private *priv,
		  const struct pciserial_board *board,
		  struct uart_8250_port *port, int idx);
static int pci_xr17vx35x_setup(struct serial_private *priv,
		  const struct pciserial_board *board,
		  struct uart_8250_port *port, int idx);
...
static int
pci_xr17vx35x_setup(struct serial_private *priv,
		  const struct pciserial_board *board,
		  struct uart_8250_port *port, int idx)
{
	port->port.flags |= UPF_EXAR_EFR;
	// Slave ports have a different baud base
	if (idx >= 8) {
		port->port.uartclk = 62500000;
	}

	return pci_xr17v35x_setup(priv, board, port, idx);
}
...
struct xr17v35x_gpio_chip {
    struct gpio_chip    chip;
    spinlock_t  lock;
    void __iomem    *membase;
};

/* Pins 16..31 are on the slave device */
#define GPIO_REG(membase,pin,loreg,hireg) \
	((membase) + ((pin)&16?0x2000:0x0000) + (((pin)&8)?hireg:loreg))
#define GPIO_LVL_REG(membase, pin) GPIO_REG(membase, pin, 0x90, 0x96)
#define GPIO_POL_REG(membase, pin) GPIO_REG(membase, pin, 0x92, 0x98)
#define GPIO_DIR_REG(membase, pin) GPIO_REG(membase, pin, 0x93, 0x99)
#define GPIO_PIN_IDX(pin) ((pin) & 7)

static void xr17v35x_gpio_set(struct gpio_chip *chip, unsigned pin, int value) {
    struct xr17v35x_gpio_chip *xvchip = 
        container_of(chip, struct xr17v35x_gpio_chip, chip);
    unsigned long flags;
    u8 u;
    spin_lock_irqsave(&xvchip->lock, flags);
    u = readb(GPIO_LVL_REG(xvchip->membase, pin));
    if (value) 
        u |= 1 << GPIO_PIN_IDX(pin);
    else 
        u &= ~(1 << GPIO_PIN_IDX(pin));
    writeb(u, GPIO_LVL_REG(xvchip->membase, pin));
    spin_unlock_irqrestore(&xvchip->lock, flags);

}

static int xr17v35x_gpio_get(struct gpio_chip *chip, unsigned pin) {
    struct xr17v35x_gpio_chip *xvchip = 
        container_of(chip, struct xr17v35x_gpio_chip, chip);
    u8 u;
    /* FIXME: Polarity support */
    u = readb(GPIO_LVL_REG(xvchip->membase, pin));
    return !!(u & (1 << GPIO_PIN_IDX(pin)));
}


static int xr17v35x_gpio_direction_input(struct gpio_chip *chip, unsigned pin) {
    struct xr17v35x_gpio_chip *xvchip = 
        container_of(chip, struct xr17v35x_gpio_chip, chip);
    unsigned long flags;
    u8 u;
    spin_lock_irqsave(&xvchip->lock, flags);
    u = readb(GPIO_DIR_REG(xvchip->membase, pin));
    u |= 1 << GPIO_PIN_IDX(pin);
    writeb(u, GPIO_DIR_REG(xvchip->membase, pin));
    spin_unlock_irqrestore(&xvchip->lock, flags);
    return 0;
}

static int xr17v35x_gpio_direction_output(struct gpio_chip *chip, unsigned pin, int value) {
    struct xr17v35x_gpio_chip *xvchip = 
        container_of(chip, struct xr17v35x_gpio_chip, chip);
    unsigned long flags;
    u8 u;
    xr17v35x_gpio_set(chip, pin, value);
    spin_lock_irqsave(&xvchip->lock, flags);
    u = readb(GPIO_DIR_REG(xvchip->membase, pin));
    u &= ~(1 << GPIO_PIN_IDX(pin));
    writeb(u, GPIO_DIR_REG(xvchip->membase, pin));
...
static int xr17v35x_gpio_init(struct pci_dev *dev) {
    struct xr17v35x_gpio_chip *xvchip;

    xvchip = devm_kzalloc(&dev->dev, sizeof(struct xr17v35x_gpio_chip), GFP_KERNEL);
    if (!xvchip)
        return -ENOMEM;


	xvchip->membase = pci_ioremap_bar(dev, 0);
	if (xvchip->membase == NULL)
		return -ENOMEM;

    xvchip->chip.label = dev_name(&dev->dev);
    xvchip->chip.dev = &dev->dev;
    xvchip->chip.direction_input = xr17v35x_gpio_direction_input;
    xvchip->chip.direction_output = xr17v35x_gpio_direction_output;
    xvchip->chip.get = xr17v35x_gpio_get;
    xvchip->chip.set = xr17v35x_gpio_set;
    /* Explicitly disable the slave port GPIOs. We don't use them for anything
     * and they cause a GPIO address space collision with the IO Expanders.
     * It is less risk disabling the extra GPIOs here, than renumbering the
     * IO Expander GPIOs in multiple places.
     */
#if 0
    xvchip->chip.ngpio = (dev->device & 0xf000) ? 32 : 16;   /* got slave? */
#else
    xvchip->chip.ngpio = 16;
#endif
    xvchip->chip.base = -1; 
    xvchip->chip.can_sleep = false;

    spin_lock_init(&xvchip->lock);
    gpiochip_add(&xvchip->chip);

    pr_info("%s: %d GPIOs, base %d\n", xvchip->chip.label,
	xvchip->chip.ngpio, xvchip->chip.base);
#ifdef CONFIG_EXAR_RS485_SUPPORT
    pr_info("PINS 0-7 used for RS422/485 switching\n");
#endif
    return 0;
}
		  ...
		  struct uart_8250_port *port, int idx)
{
	...
	membase = pci_ioremap_bar(priv->dev, 0);
	if (membase == NULL)
		return -ENOMEM;

	port->port.flags |= UPF_EXAR_EFR;
	p = membase + (idx << 10); /* 0x400 per channel */
...
#ifdef CONFIG_EXAR_RS485_SUPPORT
    {
        ...
        /* Find the GPIO chip that is attached to this device */
        chip = gpiochip_find((void *)dev_name(&priv->dev->dev), match_name);
        if (chip) {
            port->port.rs485_support = 1;
            gpio_request_one(chip->base + idx, GPIOF_DIR_OUT, "");
            gpio_set_value(chip->base + idx, false);
            port->port.rs485_gpio = chip->base + idx;
            port->port.serial_mode = SERIAL_MODE_RS232;
        } else {
            pr_err("Could not find GPIO chip for RS485 support\n");
        }
    }
#endif
