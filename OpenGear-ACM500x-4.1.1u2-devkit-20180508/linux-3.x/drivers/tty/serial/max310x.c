...
static noinline void max310x_fifo_read(struct uart_port *port, size_t rxlen) {
	struct max310x_port *s = dev_get_drvdata(port->dev);
	regcache_cache_bypass(s->regmap, true);
	regmap_raw_read(s->regmap, port->iobase + MAX310X_RHR_REG , s->p->fifo, rxlen);
	regcache_cache_bypass(s->regmap, false);
}

static noinline void max310x_fifo_write(struct uart_port *port, size_t txlen) {
	struct max310x_port *s = dev_get_drvdata(port->dev);
	regcache_cache_bypass(s->regmap, true);
	regmap_raw_write(s->regmap, port->iobase + MAX310X_THR_REG, s->p->fifo, txlen);
	regcache_cache_bypass(s->regmap, false);
}
...
static noinline void max310x_port_write(struct uart_port *port, u8 reg, u8 val)
{
	struct max310x_port *s = dev_get_drvdata(port->dev);
...
static void max310x_handle_rx(struct uart_port *port, unsigned int rxlen, unsigned int ists)
{
	struct max310x_port *s = dev_get_drvdata(port->dev);
	unsigned int ch, flag;
	unsigned int i = 0, sts = 0, bytes_read = 0;
	bool read_lsr_sts = (ists & MAX310X_IRQ_LSR_BIT) ? 1 : 0; 
	...
	while (rxlen) {
		/* Only read lsr status if there are possible FIFO Errors */
		if (read_lsr_sts) {
		...
		if (read_lsr_sts) {
			s->p->fifo[0] = max310x_port_read(port, MAX310X_RHR_REG);
			bytes_read = 1;
		} else {
...
static void max310x_handle_tx(struct uart_port *port)
{
	...
	if (likely(to_send)) {
		...
		//Kenw - test starvation
		to_send = (to_send > 32) ? 32 : to_send;
		/* Add data to send */
		port->icount.tx += to_send;
		/* Convert to linear buffer */
		for (i = 0; i < to_send; ++i) {
			s->p->fifo[i] = xmit->buf[xmit->tail];
			xmit->tail = (xmit->tail + 1) & (UART_XMIT_SIZE - 1);
		}
		max310x_port_update(port, MAX310X_MODE1_REG,
				MAX310X_MODE1_TXDIS_BIT, MAX310X_MODE1_TXDIS_BIT); 
		max310x_fifo_write(port, to_send);
		max310x_port_update(port, MAX310X_MODE1_REG,
...
static void max310x_port_irq(struct max310x_port *s, int portno)
{
	...
	do {
		...
		if (ists & MAX310X_IRQ_LSR_BIT) {
			lsr = max310x_port_read(port, MAX310X_LSR_IRQSTS_REG);
		}
		if (rxlen)
			max310x_handle_rx(port, rxlen, ists);
...
static noinline irqreturn_t max310x_ist(int irq, void *dev_id)
{
...
static unsigned int max310x_get_mctrl(struct uart_port *port)
{
	 ...
	 */
	/* Handle DTR/DSR and DTD */
	return TIOCM_DSR | TIOCM_CAR;
#else
	/* Get DSR and DCD */
	int dsr, dcd, lsr, ret;
	struct max310x_one *one = container_of(port, struct max310x_one, port);
	//dsr = gpio_get_value_cansleep(one->dsr_gpio);
	//dcd = gpio_get_value_cansleep(one->dcd_gpio);
	//lsr = max310x_port_read(port, MAX310X_LSR_IRQSTS_REG);

	ret = 0;
	ret |= (dsr ? TIOCM_DSR : 0);
	ret |= (dcd ? TIOCM_CAR : 0);
	ret |= (lsr & MAX310X_LSR_CTS_BIT ? TIOCM_CTS : 0);
...
static void max310x_md_proc(struct work_struct *ws)
{
	...
	max310x_port_update(&one->port, MAX310X_LCR_REG,
			    MAX310X_LCR_RTS_BIT,
			    (one->port.mctrl & TIOCM_RTS) ?
			    MAX310X_LCR_RTS_BIT : 0);
	gpio_set_value_cansleep(one->dtr_gpio, (one->port.mctrl & TIOCM_DTR) ? 1 : 0);
}
...
static int max310x_startup(struct uart_port *port)
{
	...
	/* Set the RX timeout register to 96 chars */
	max310x_port_write(port, MAX310X_RXTO_REG, 96);
	/* Generate LSR interrupts when RX timeouts occur */
	max310x_port_write(port, MAX310X_LSR_IRQEN_REG, MAX310X_LSR_RXTO_BIT);

	/* Clear IRQ status register */
	max310x_port_read(port, MAX310X_IRQSTS_REG);

	/* Enable RX, TX, CTS change, and LSR interrupts */
			 ...
			 struct regmap *regmap, int irq, unsigned long flags)
{
	...
	/* NOTE: Resetting a port does bad things if it's been initialised in u-boot 
	 * We hardware reset on boot, so no need to software reset */
#if 0
	...
	for (i = 0; i < devtype->nr; i++) {
		...
		/* Assign GPIOs */
		s->p[i].dtr_gpio = s->gpio.base + (i * 4);
		s->p[i].dsr_gpio = s->gpio.base + (i * 4) + 1;
		s->p[i].dcd_gpio = s->gpio.base + (i * 4) + 2;
		if (i == 0)
			s->p[i].rs485_gpio = s->gpio.base + (i * 4) + 3;
		else 
			s->p[i].rs485_gpio = -ENOENT;

		sprintf(gpio_name_buf, "UART %d DTR", i);
		gpio_request(s->p[i].dtr_gpio, gpio_name_buf);
		gpio_direction_output(s->p[i].dtr_gpio, 0);

		sprintf(gpio_name_buf, "UART %d DSR", i);
		gpio_request(s->p[i].dsr_gpio, gpio_name_buf);

		sprintf(gpio_name_buf, "UART %d DCD", i);
		gpio_request(s->p[i].dcd_gpio, gpio_name_buf);

		if (s->p[i].rs485_gpio != -ENOENT) {
			sprintf(gpio_name_buf, "UART %d RS485", i);
			gpio_request(s->p[i].rs485_gpio, gpio_name_buf);
			gpio_direction_output(s->p[i].rs485_gpio, 0);
		}
		/* Disable all interrupts */
...
static int max310x_remove(struct device *dev)
{
	...
	for (i = 0; i < s->uart.nr; i++) {
		gpio_free(s->p[i].dtr_gpio);
		gpio_free(s->p[i].dsr_gpio);
		gpio_free(s->p[i].dcd_gpio);
		if (s->p[i].rs485_gpio != -ENOENT) {
			gpio_free(s->p[i].rs485_gpio);
		}
