...
static unsigned int og_scan[] = {
		0x0, LED_D1, LED_D2, LED_D3, LED_D4, LED_D3, LED_D2, LED_D1
};
...
static ledmap_t	og_std = {
	...
	[LEDMAN_WIFI_TX]	= LED_D1,
	[LEDMAN_NVRAM_1]	= LED_D4 | LED_D2,
	[LEDMAN_NVRAM_2]	= LED_D3 | LED_D1
};
...
/*
 *	Definitions of the LED's on the OpenGear/ACM550X circuit board, as
 *	per the labels next to them. There are 4 software controlled LEDs.
 */
#define	LED_D1	0x1
#define	LED_D2	0x2
...
static ledmap_t	og_std = {
	...
	[LEDMAN_LAN1_RX]	= LED_D4,
	[LEDMAN_LAN1_TX]	= LED_D4,
	[LEDMAN_LAN2_RX]	= LED_D4,
	[LEDMAN_LAN2_TX]	= LED_D4,
	[LEDMAN_WIFI_TX]	= LED_D5,
	[LEDMAN_MODEM]		= LED_D5,
	[LEDMAN_USB1_TX]	= LED_D6,
	[LEDMAN_CELLULAR]	= LED_D6,
	[LEDMAN_NVRAM_1]	= LED_D1 | LED_D3 | LED_D5,
	[LEDMAN_NVRAM_2]	= LED_D2 | LED_D4 | LED_D6
};
...
extern acm550x_set_leds(unsigned long bits);
static void og_set(unsigned long bits)
{
	acm550x_set_leds(bits);
}

static void ledman_initarch(void)
{
	u32 inter = (LOW_IRQS + KS8692_INT_EXT_INT3);
	/* Configure IRQ5 as our erase button interrupt */
	if (request_irq(inter, ledman_interrupt, IRQF_DISABLED, "Erase", NULL))
		printk("LED: failed to register IRQ%d for ERASE switch\n", inter);
	else
...
static ledmap_t	og_std = {
	...
	[LEDMAN_LAN2_RX]	= LED_D4,
	[LEDMAN_LAN2_TX]	= LED_D4,
	[LEDMAN_NVRAM_1]	= LED_D1 | LED_D3,
	[LEDMAN_NVRAM_2]	= LED_D2 | LED_D4
};
...
static bool im72xx_leds_initialised = false;
static unsigned long im72xx_ledbits = 0;
static int ledman_button;
static struct timer_list ledman_timer;

static void led_workqueue_func(struct work_struct *ws) {

	gpio_set_value_cansleep(GPIO_POWER_LED, (im72xx_ledbits & LED_D1) ? 1 : 0); 
	gpio_set_value_cansleep(GPIO_HEART_BEAT_LED, (im72xx_ledbits & LED_D2) ? 1 : 0); 
	gpio_set_value_cansleep(GPIO_SERIAL_ACTIVITY_LED, (im72xx_ledbits & LED_D3) ? 1 : 0); 
	gpio_set_value_cansleep(GPIO_NETWORK_ACTIVITY_LED, (im72xx_ledbits & LED_D4) ? 1 : 0); 
}

static void erase_workqueue_func(struct work_struct *ws) {
	int val = gpio_get_value_cansleep(GPIO_CONFIG_RESET);
	if (val == 1) {
		if (ledman_button == 0) {
			pr_info("reset button pushed!\n");
			ledman_signalreset();
	...
	/* Re-arm timer interrupt. */
	mod_timer(&ledman_timer, jiffies + HZ/5);
}

static DECLARE_WORK(im72xx_ledwork, led_workqueue_func);
static DECLARE_WORK(im72xx_erasework, erase_workqueue_func);

static void ledman_erase_schedwork(unsigned long arg)
{
	schedule_work(&im72xx_erasework);
}
void ledman_leds_initialised() {
	int ret;
	printk("LED: LEDs initialised\n");
	im72xx_leds_initialised = true;
	ret = gpio_request(GPIO_CONFIG_RESET, "Erase");
	if (ret == 0) {
		gpio_direction_input(GPIO_CONFIG_RESET);
		pr_info("LED: registered ERASE switch (polled)\n");
		ledman_button = 1;
		ledman_erase_schedwork(0);
	}
}
static void og_set(unsigned long bits)
{
	im72xx_ledbits = bits;
	if (im72xx_leds_initialised) {
		schedule_work(&im72xx_ledwork);
	}
}
...
static void ledman_initarch(void)
{
	...
	og_set(0);
	init_timer(&ledman_timer);
	ledman_timer.function = ledman_erase_schedwork;
	ledman_timer.data = 0;
}
...
static struct ledmode led_mode[] = {
    ...
    defined(CONFIG_MACH_ACM550X) || defined(CONFIG_MACH_IM72xx)
