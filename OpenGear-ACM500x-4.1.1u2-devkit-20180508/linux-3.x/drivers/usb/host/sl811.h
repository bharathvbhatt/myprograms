 ...
 * readX/writeX code.
 */
static void invalidate_bus_buffer(void)
{
        static volatile unsigned char *fp = 0;
        unsigned int val;

        if (fp == 0) {
		/* This is the flash region - must be real external I/O */
                fp = ioremap(0x03800000, 4096);
        } else {
                val = *fp;
        }
}
