...
/*
 *  Remove windows, starting from the largest ones to the smalles ones.
 */
 ...
 */
static void mvebu_pcie_add_windows_remap_flags(struct mvebu_pcie_port *port,
        phys_addr_t base, size_t size,
        phys_addr_t remap, int flags)
{
    ...
    while (size) {
        ...
        ret = mvebu_mbus_add_window_remap_flags(port->name, base,
                sz, remap, flags);
        if (ret) {
...
static void mvebu_pcie_handle_iobase_change(struct mvebu_pcie_port *port)
{
	...
	mvebu_pcie_add_windows_remap_flags(port, port->iowin_base,
					  port->iowin_size,
					  iobase,
					  MVEBU_MBUS_PCI_IO);
...
static void mvebu_pcie_handle_membase_change(struct mvebu_pcie_port *port)
{
	...
	mvebu_pcie_add_windows_remap_flags(port, port->memwin_base,
					  port->memwin_size,
					  MVEBU_MBUS_NO_REMAP,
					  MVEBU_MBUS_PCI_MEM);
}
					  ...
					  resource_size_t align)
{
	...
	if (res->flags & IORESOURCE_IO)
		return round_up(start, max((resource_size_t)SZ_64K, rounddown_pow_of_two(size)));
	else if (res->flags & IORESOURCE_MEM)
		return round_up(start, max((resource_size_t)SZ_1M, rounddown_pow_of_two(size)));
	else
		return start;
}
