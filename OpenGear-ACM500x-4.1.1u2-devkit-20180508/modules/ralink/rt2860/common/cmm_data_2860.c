...
#ifdef SPECIFIC_MEM_MGMT
static inline VOID RTMPSetSDfromTxBlk(
	...
	IN	UCHAR			SDidx)
{
	if (SDidx == 0)
	{
		pTxD->SDPtr0 = PCI_MAP_SINGLE(pAd, (char *) pSrcBufData, SrcBufLen, PCI_DMA_TODEVICE);
		pTxD->SDLen0 = SrcBufLen;
	}
	else
	{
		pTxD->SDPtr1 = PCI_MAP_SINGLE(pAd, (char *) pSrcBufData, SrcBufLen, PCI_DMA_TODEVICE);
		pTxD->SDLen1 = SrcBufLen;
	}
}
#endif // SPECIFIC_MEM_MGMT //
...
#ifdef SPECIFIC_MEM_MGMT
	RTMPSetSDfromTxBlk(pAd, pTxD, pTxBlk->QueIdx, TxIdx, pTxBlk->pSrcBufData, pTxBlk->SrcBufLen, 1);
#else
	...
	IN UINT 			SrcBufLen)
{
	...
	pTxD->SDLen1 = 0;
#ifdef SPECIFIC_MEM_MGMT
	RTMPSetSDfromTxBlk(pAd, pTxD, QueIdx, SwIdx, pSrcBufVA, SrcBufLen, 0);
#else	
	...
	IN USHORT           TbttNumToNextWakeUp) 
{
	...
	{	
		// Must using 40MHz.
		RT28xxPciAsicTurnOffRFClk(pAd, pAd->CommonCfg.CentralChannel);
	}
	else
	{	
		// Must using 20MHz.
		RT28xxPciAsicTurnOffRFClk(pAd, pAd->CommonCfg.Channel);
	}
