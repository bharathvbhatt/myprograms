	...
	IN UCHAR			idx)
{
	...
	for (i=0; i<TXWI_SIZE; i+=4)  // 24-byte TXINFO field
	{
		...
		RTMP_IO_WRITE32(pAd, pAd->BeaconOffset[idx] + i, longValue);
		ptr +=4;
	}
	...
	{
		longValue =  *ptr + (*(ptr+1)<<8) + (*(ptr+2)<<16) + (*(ptr+3)<<24);
		RTMP_IO_WRITE32(pAd, pAd->BeaconOffset[idx] + TXWI_SIZE + i, longValue);
		ptr += 4;
	}
	...
	IN UCHAR			idx) 
{
	...
	*(ptr+1) = (UCHAR)((pAd->MeshTab.CapabilityInfo & 0xff00) >> 8);
	...
	//	
	RT28xx_UpdateBeaconToAsic(pAd, idx, FrameLen, UpdatePos);
}
