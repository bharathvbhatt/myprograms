...
#include <cryptodev.h>
#include <uio.h>
#include <linux/platform_data/mv_cesa.h>
...
struct mv_cesa_platform_data dummy_data;
...
mv_cesa_ocf_init(struct platform_device *pdev)
{
	...
	/* Get the IRQ, and crypto memory regions */
	{
        ...
        if (pdev->dev.of_node)
            cesa_device.irq = irq_of_parse_and_map(pdev->dev.of_node, 0);
        else {
        ...
        /* Armada 370 has single channel CESA */
        chan = 0;
        hal_data.sramVirtBase[chan] = cesa_device.sram;
        hal_data.sramPhysBase[chan] = virt_to_phys(cesa_device.sram);
        hal_data.regBase[chan] = cesa_device.reg;
        hal_data.irq[chan] = cesa_device.irq;
        hal_data.sramOffset[chan] = 0;
        hal_data.ctrlModel = 0x6711;
        hal_data.ctrlRev = 0x0;
		// Get the DRAM window
		dummy_data.dram = mv_mbus_dram_info();
		cesa_device.plat_data = &dummy_data;
	        setup_tdma_mbus_windows(&cesa_device);
    }
	if( MV_OK != mvCesaIfInit(CESA_OCF_MAX_SES*5, CESA_Q_SIZE, NULL, &hal_data) ) {
	...
	for(chan = 0; chan < MV_CESA_CHANNELS; chan++) {
		...
		if( request_irq( cesa_device.irq, cesa_interrupt_handler,
				(IRQF_DISABLED) , irq_str[chan], &chan_id[chan]) < 0) {
            		printk("%s,%d: cannot assign irq %x\n", __FILE__, __LINE__, cesa_device.irq);
			return EINVAL;
        	}
	}
